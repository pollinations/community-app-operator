...
const CATALOG_FILE = path.resolve(
    process.env.APP_CATALOG_PATH || "workspace/catalog.json",
);
const CATALOG_URL =
    process.env.APP_CATALOG_URL ||
    "https://raw.githubusercontent.com/pollinations/pollinations/main/apps/catalog.json";
...
async function fetchCatalog(filePath = CATALOG_FILE) {
    const response = await fetch(CATALOG_URL);
    if (!response.ok) {
        throw new Error(`Catalog download returned HTTP ${response.status}`);
    }
    const apps = validateApps(await response.json(), CATALOG_URL);
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
    writeApps(apps, filePath);
    return apps;
}
...
async function main() {
    ...
    if (command === "fetch") {
        ...
        console.log(`Fetched ${apps.length} apps to ${CATALOG_FILE}`);
        return;
    }
    if (command === "validate") {
        const apps = readApps();
    ...
    throw new Error("Expected fetch or validate command");
}
