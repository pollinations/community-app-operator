...
const SCREENSHOT_AGENT_SYSTEM_PROMPT = fs
    .readFileSync(
        path.join(__dirname, "../prompts/reviewer.system.txt"),
        "utf8",
    )
    .trim();
...
async function main() {
    ...
    if (process.argv.includes("--publish")) {
        throw new Error(
            "Review runs never publish. Edit review.json, then use npm run apply.",
        );
    }
        ...
        `${JSON.stringify(
            {
                decisions: captures
                    .filter((result) =>
                        ["keep", "remove"].includes(result.outcome),
                    )
                    .map((result) => ({
                        apply: false,
                        name: result.name,
    ...
    console.log(`Manual decisions: ${reviewPath}`);
}
